...
train_data_path = '/home/sabbels/Spoken-SQuAD-master/spoken_train-v1.1.json'
test_data_path = '/home/sabbels/Spoken-SQuAD-master/spoken_test-v1.1.json'
fig_save_path = 'figs'
        ...
        for paragraph in group['paragraphs']:
            ...
            for qa in paragraph['qas']:
                ...
                for answer in qa['answers']:
                    con.append(context.lower())  
                    que.append(question.lower()) 
                    answ.append(answer)          
    return con, que, answ
...
def start_and_end(idx):
    ...
    for a in range( len(train_encodings['input_ids'][idx]) -  len(answer_encoding_fast['input_ids']) ): 
        match = True
...
for epoch in range(EPOCHS):
    ...
    for i in range(len(answer_list)):
        ...
        if(len(answer_list[i][1])==0):
            answer_list[i][1]="$"
        pred_answ.append(answer_list[i][0])
        true_answ.append(answer_list[i][1])
    wer_score = wer.compute(predictions=pred_answ, references=true_answ)
